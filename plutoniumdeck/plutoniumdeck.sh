...
tutorial() {
  ...
  say ""
  cat_line "$UI_ML"
  p "$UI_ML" "Step 1: Download Game Files via Torrents

After running the Plutonium installer, locate the torrent files in ~/wine/plutonium/callofduty/.
...
Repeat Step 4 every launch. Spoofy must be active to bypass the ban."
  ...
  p "$UI_NOTE" "  lutris/* (contents)-> ~/.local/share/lutris/games/"
  say ""
}
...
p "$UI_INFO" "Copying lutris/ contents -> $DEST_LUTRIS_GAMES/"
copy_dir_contents "$SRC_LUTRIS" "$DEST_LUTRIS_GAMES"

say ""
say "$BANNER_LINE"
p "$UI_OK" "Done."
p "$UI_OK" "Installed to: $DEST_PLUTONIUM"
p "$UI_OK" "Lutris games updated: $DEST_LUTRIS_GAMES"
say "$BANNER_LINE"
